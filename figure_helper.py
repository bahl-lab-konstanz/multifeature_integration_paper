...
class Figure():
    def __init__(self, **opts_dict):
            ...
            Parameters:
                **opts_dict (keyword arguments): Additional options for the plotter.
                    ...
                    - helper_lines_lc: str, optional
                        Color of horizontal and vertical helper lines, and polar grids. Default is 'darkgray'.

                    - xl_distance: float, optional
                        Distance (in cm, unless xyzl_3d=True) of the xlabel from the axes. Default is 0.75.

                    - yl_distance: float, optional
                        Distance (in cm, unless xyzl_3d=True) of the ylabel from the axes. Default is 0.75.

                    - zl_distance: float, optional
                        Distance (in cm, unless xyzl_3d=True) of the zlabel from the axes. Default is 0.75.
                    ...
                    - rasterized: bool, optional
                        If True, display plots as pixel rasters for improved rendering and loading of huge data clouds. Default is False.

                    - axes_lw: float, optional
                        Line width of all plot borders. Helper lines will be half of this value. Default is 0.5.
                    ...
                    - xticks, yticks, zticks: list or None, optional
                        Custom tick positions for the x, y, and z axes. Default is None (auto-generated ticks).
                    ...
                    - xticklabels_rotation, yticklabels_rotation, zticklabels_rotation: float, optional
                        Rotation angle (in degrees) for tick labels on the x, y, and z axes. Default is 0.
                    ...
                    - vspans, hspans: list or None, optional
                        List of vertical and horizontal spans (rectangles) to add to the plot. Default is None.
                    ...
                    - helper_lines_alpha: float, optional
                        Transparency of horizontal and vertical helper lines, and polar grids. Default is 1.0.
        ...
        self.figure_dict = {
                ...
                'dash_capstyle':"round",
                 ...
                 "xl_distance": 0.75,  # Distance (in cm, unless xyzl_3d=True) of the xlabel from the axes
                 "yl_distance": 0.75,  # Distance (in cm, unless xyzl_3d=True) of the ylabel from the axes
                 "zl_distance": 0.75,  # Distance (in cm, unless xyzl_3d=True) of the zlabel from the axes
                 'xyzl_3d': False,  # Boolean, if true the xl, yl, zl distances are based on the 3d rotation.
        ...
        for key in opts_dict.keys():
            if key not in self.figure_dict.keys():
        ...
        self.fig = plt.figure(num=None, facecolor=self.figure_dict["facecolor"], edgecolor='none', dpi=self.figure_dict["dpi"])

        # Somhow on MAC, one needs to do this
        self.fig.set_size_inches(self.figure_dict["fig_width"] / 2.54, self.figure_dict["fig_height"] / 2.54)

        if self.figure_dict["figure_title"] is not None:
            plt.figtext(0.5, 0.95, self.figure_dict["figure_title"], ha='center', va='center', fontsize=self.figure_dict["fontsize_figure_title"], color=self.figure_dict["textcolor"])
    ...
    def add_text(self, x, y, text, rotation=0, ha='center'):
        plt.figtext(x / self.figure_dict["fig_width"], y / self.figure_dict["fig_height"], text, ha=ha, ma=ha, va='center',
...
class SuperPlot():
    ...
    def set_axes_properties(self):
        ...
        if self.plot_dict["zl"] is not None and self.plot_dict["show_colormap"] is None:
            self.ax.set_zlabel(self.plot_dict["zl"], verticalalignment='center', horizontalalignment='center',
                               color=self.plot_dict["textcolor"])
    ...
    def draw_rectangle(self, x, y, width, height, fill, **opts_dict):
        ...
        if artist_dict["line_dashes"] is not None:
            rect = patches.Rectangle((x, y), width, height, fill=fill,
                                     ...
                                     linestyle=(0, artist_dict["line_dashes"]), capstyle="round",
    ...
    def draw_circle(self, x, y, r, fill, **opts_dict):
        ...
        self.current_zorder += 1
        if artist_dict["line_dashes"] is not None:
            circle = plt.Circle((x, y), r, fill=fill, edgecolor=artist_dict["lc"], fc=artist_dict["fc"],
                                linestyle=(0, artist_dict["line_dashes"]), capstyle="round",
                                alpha=artist_dict["alpha"], lw=artist_dict["lw"], rasterized=artist_dict["rasterized"],
                                zorder=self.current_zorder)
        else:
            circle = plt.Circle((x, y), r, fill=fill, edgecolor=artist_dict["lc"], fc=artist_dict["fc"],
                                alpha=artist_dict["alpha"], lw=artist_dict["lw"],
    ...
    def draw_line(self, x, y, **opts_dict):
        ...
        if  artist_dict['yerr'] is not None:
            ...
            if artist_dict["errorbar_area"] is False:
                ...
                self.ax.fill_between(x, y - yerr, y + yerr, lw=artist_dict["ealw"], edgecolor=artist_dict["eaec"], facecolor=artist_dict["eafc"], alpha=artist_dict["eaalpha"],
                                     rasterized=artist_dict["rasterized"], zorder=self.current_zorder)

        if  artist_dict['yerr_pos'] is not None and artist_dict['yerr_neg'] is not None:
            ...
            if artist_dict["errorbar_area"] is False:
                self.ax.errorbar(x, y, yerr=[yerr_neg, yerr_pos], elinewidth=artist_dict["elw"], ecolor=artist_dict["lc"], fmt='none', capsize=1.5*artist_dict["elw"], mew=artist_dict["elw"],
                                 solid_capstyle='round', solid_joinstyle='round', rasterized=artist_dict["rasterized"], zorder=self.current_zorder)
            else:
                self.ax.fill_between(x, y - yerr_neg, y + yerr_pos, lw=artist_dict["ealw"], edgecolor=artist_dict["eaec"], facecolor=artist_dict["eafc"], alpha=artist_dict["eaalpha"],
                                 rasterized=artist_dict["rasterized"], zorder=self.current_zorder)
        ...
        if artist_dict["xerr"] is not None:
            ...
            if artist_dict["errorbar_area"] is False:
                ...
                self.ax.fill_betweenx(y, x + xerr, x - xerr, lw=artist_dict["ealw"], edgecolor=artist_dict["eaec"], facecolor=artist_dict["eafc"], alpha=artist_dict["eaalpha"],
                                      rasterized=artist_dict["rasterized"], zorder=self.current_zorder)

        self.current_zorder += 1

        if artist_dict["line_dashes"] is not None:
            self.ax.plot(x, y, color=artist_dict["lc"], lw=artist_dict["lw"], alpha=artist_dict["alpha"], dashes=artist_dict["line_dashes"], dash_capstyle=artist_dict["dash_capstyle"],
    ...
    def draw_polygon(self, x, y, **opts_dict):
        ...
        if artist_dict["line_dashes"] is not None:
            ...
            self.ax.plot(x, y, color=artist_dict["lc"], lw=artist_dict["lw"], dashes=artist_dict["line_dashes"], dash_capstyle=artist_dict["dash_capstyle"], rasterized=artist_dict["rasterized"], zorder=self.current_zorder)
    ...
    def draw_scatter(self, x, y, **opts_dict):
        ...
        if artist_dict['yerr_pos'] is not None and artist_dict['yerr_neg'] is not None:
            ...
            yerr_neg = np.array(artist_dict["yerr_neg"])
            self.current_zorder += 1
            self.ax.errorbar(x, y, yerr=[yerr_neg, yerr_pos], elinewidth=artist_dict["elw"], ecolor=artist_dict["ec"], fmt='none',
                             capsize=1.5*artist_dict["elw"], mew=artist_dict["elw"],
                             solid_capstyle='round', solid_joinstyle='round', zorder=self.current_zorder)
    ...
    def draw_scatter3D(self, x, y, z, **opts_dict):
        ...
        if artist_dict["yerr"] is not None:
            self.current_zorder += 1
            self.ax.errorbar(x, y, z, yerr=artist_dict["yerr"], elinewidth=artist_dict["elw"], ecolor=artist_dict["ec"], fmt='none',
                             capsize=1.5*artist_dict["elw"], mew=artist_dict["elw"],
                             solid_capstyle='round', solid_joinstyle='round', zorder=self.current_zorder)

        if artist_dict["xerr"] is not None:
            self.current_zorder += 1
            self.ax.errorbar(x, y, z, xerr=artist_dict["xerr"], elinewidth=artist_dict["elw"], ecolor=artist_dict["ec"], fmt='none',
                             capsize=1.5*artist_dict["elw"], mew=artist_dict["elw"],
                             solid_capstyle='round', solid_joinstyle='round', zorder=self.current_zorder)

        if artist_dict["zerr"] is not None:
            self.current_zorder += 1
            self.ax.errorbar(x, y, z, zerr=artist_dict["zerr"], elinewidth=artist_dict["elw"], ecolor=artist_dict["ec"], fmt='none',
                             capsize=1.5*artist_dict["elw"], mew=artist_dict["elw"],
                             solid_capstyle='round', solid_joinstyle='round', zorder=self.current_zorder)
        ...
        if isinstance(artist_dict["pc"], list) or isinstance(artist_dict["pc"], np.ndarray):
            if artist_dict["norm_colormap"] is None:
                self.ax.scatter(x, y, z, c=artist_dict["pc"], marker=artist_dict["pt"], s=artist_dict["ps"] ** 2, linewidths=artist_dict["elw"], edgecolor=artist_dict["ec"], alpha=artist_dict["alpha"], label=artist_dict["label"],
                                rasterized=artist_dict["rasterized"], cmap=artist_dict["colormap"], norm=None, vmin=artist_dict["vmin"], vmax=artist_dict["vmax"], zorder=self.current_zorder)
            else:

                self.ax.scatter(x, y, z, c=artist_dict["pc"], marker=artist_dict["pt"], s=artist_dict["ps"] ** 2,
                                linewidths=artist_dict["elw"], edgecolor=artist_dict["ec"], alpha=artist_dict["alpha"],
                                label=artist_dict["label"],
                                rasterized=artist_dict["rasterized"], cmap=artist_dict["colormap"], norm=artist_dict["norm_colormap"],
                                zorder=self.current_zorder)
    ...
    def draw_vertical_bars(self, x, y, **opts_dict):
        ...
        if type(artist_dict['vertical_bar_bottom']) is not list:
            artist_dict['vertical_bar_bottom'] = [artist_dict['vertical_bar_bottom']] * len(x)
        ...
        for i in range(len(x)):
            ...
            if artist_dict["bl"] is not None:
                if not np.isnan(y[0]) and not np.isnan(x[0]):
                    ...
                    plt.figtext(x_, y_, artist_dict["bl"], ha='center', va='center', color=artist_dict["textcolor"])
    ...
    def draw_horizontal_bars(self, x, y, **opts_dict):
        ...
        if type(artist_dict['horizontal_bar_left']) is not list:
            artist_dict['horizontal_bar_left'] = [artist_dict['horizontal_bar_left']] * len(x)

        for i in range(len(x)):
            self.current_zorder += 1
            self.ax.barh(y[i], x[i], left=artist_dict['horizontal_bar_left'][i], edgecolor=None, lw=None, facecolor=artist_dict["lc"][i], alpha=artist_dict["alpha"][i], align='center',
                         rasterized=True,
    ...
    def draw_image(self, img, extent, **opts_dict):
        ...
        self.current_zorder += 1
        if artist_dict["norm_colormap"] is None:
            im = self.ax.imshow(np.array(img), extent=extent, interpolation=artist_dict["image_interpolation"], origin=artist_dict["image_origin"], aspect='auto',
                           cmap=plt.get_cmap(artist_dict["colormap"]), vmin=artist_dict["zmin"], vmax=artist_dict["zmax"], alpha=artist_dict["alpha"],
                           rasterized=artist_dict["rasterized"], zorder=self.current_zorder)
        else:
            im = self.ax.imshow(np.array(img), extent=extent, interpolation=artist_dict["image_interpolation"],
    ...
    def draw_pcolormesh(self, x, y, Z, aa=False, shading="gouraud", **opts_dict):
        ...
        Parameters:
            ...
            **opts_dict (keyword arguments): Additional options for the pcolormesh plot.
                ...
                - norm_colormap: object or None, optional
                    Normalization object for the colormap. If None, the colormap will not be normalized. Default is None.

                - colormap: str, optional
                    Name of the colormap to use for color mappings. Default is "inferno".

                - rasterized: bool, optional
                    If True, display the plot as a pixel raster for improved rendering and loading of huge data clouds. Default is False.
...
class PolarPlot(SuperPlot):
    def __init__(self, figure, opts_dict):
        ...
        self.ax.grid(color=self.plot_dict["helper_lines_lc"], linewidth=self.plot_dict["axes_lw"]*0.5, dashes=self.plot_dict["helper_lines_dashes"],
                     solid_capstyle="round", dash_capstyle= self.plot_dict["dash_capstyle"], zorder=self.current_zorder)
        ...
        plt.figtext((self.plot_dict["xpos"] - 0.3 * self.plot_dict["fontsize"] / 9.) / self.plot_dict["fig_width"],
    ...
    def __init__(self, figure, opts_dict):
        ...
        if self.plot_dict["plot_label"] is not None:
            if self.ax.spines['left'].get_visible():
                plt.figtext((self.plot_dict["xpos"] - 1.8 * self.plot_dict["fontsize"] / 9.) / self.plot_dict["fig_width"],
                           (self.plot_dict["ypos"] + self.plot_dict["plot_height"] + 0.5) / self.plot_dict["fig_height"], self.plot_dict["plot_label"], weight='bold',
                           fontsize=self.plot_dict["fontsize_plot_label"], ha='center', va='center', color=self.plot_dict["textcolor"])
            else:
                plt.figtext((self.plot_dict["xpos"] - 0.3 * self.plot_dict["fontsize"] / 9.) /  self.plot_dict["fig_width"],
        ...
        if self.plot_dict["show_colormap"]:
            ...
            if self.plot_dict["norm_colormap"] is None:
                cbar_ax2.imshow(np.c_[np.linspace(self.plot_dict["zmin"], self.plot_dict["zmax"], 500)],
                                ...
                                aspect='auto', origin='lower', cmap=plt.get_cmap(self.plot_dict["colormap"]), zorder=self.current_zorder)
            else:
                cbar_ax2.imshow(np.c_[np.linspace(self.plot_dict["norm_colormap"].vmin, self.plot_dict["norm_colormap"].vmax, 500)],
                                ...
                                cmap=plt.get_cmap(self.plot_dict["colormap"]), zorder=self.current_zorder)
            ...
            if self.plot_dict["zticks"] is not None:
                 ...
                 cbar_ax2.set_yticklabels(self.plot_dict["zticklabels"], rotation=self.plot_dict["zticklabels_rotation"],
                                          horizontalalignment=self.plot_dict["zticklabels_ha"],
                                          color=self.plot_dict["textcolor"])
...
class Plot3D(SuperPlot):
    def __init__(self, figure, opts_dict):
        ...
        if self.plot_dict["elev"] is not None and self.plot_dict["azim"] is not None:
            self.ax.view_init(elev=self.plot_dict["elev"], azim=self.plot_dict["azim"])
        elif self.plot_dict["elev"] is not None:
            self.ax.view_init(elev=self.plot_dict["elev"])
        elif self.plot_dict["azim"] is not None:
            self.ax.view_init(azim=self.plot_dict["azim"])
        ...
        if self.plot_dict["zticklabels"] is not None:
            self.plot_dict["zticklabels"] = self.plot_dict["zticklabels"].copy()  # make sure we do not modifiy the original

            for i in range(len(self.plot_dict["zticklabels"])):
                self.plot_dict["zticklabels"][i] = self.plot_dict["zticklabels"][i].replace("-", '–')

            self.ax.set_zticklabels(self.plot_dict["zticklabels"], rotation=self.plot_dict["zticklabels_rotation"], horizontalalignment='right',
                                    color=self.plot_dict["textcolor"])
        else:
            self.ax.spines['left'].set_visible(False)
            self.ax.tick_params(axis='z', which='minor', left='off')
            self.ax.tick_params(axis='z', which='major', left='off')
